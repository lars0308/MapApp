...
- Figuren und Builder-Zustand (localStorage `mapforge.*`) werden per Drei-Wege-Abgleich synchronisiert: `localHashes` in `mapforge.relay.sync`, `mergeGallery` in `relay.ts`, danach `reloadSpritesFromStorage()`.
...
8. ~~Godot-Terrain-Set für Wände~~ erledigt: `WALL_SIDES` und `GROUND_SOLID` in `godotScene.ts`, in Godot per `set_cells_terrain_connect` geprüft. ~~Direkter `.tscn`/`.tres`-Export~~ erledigt: `src/export/godotScene.ts` erzeugt `tile_map_data` als base64 im Format 0 (uint16 Version, dann 12 Bytes pro Zelle). Getestet mit Godot 4.3 aus dem Scratchpad.
