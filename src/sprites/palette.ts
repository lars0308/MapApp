...
export interface DrawPalette {
  ...
  name: string;
  colors: string[];
  /** built-in palettes are read-only – editing makes a copy */
  preset?: boolean;
}

const hexes = (s: string) => s.split(/\s+/).filter(Boolean).map((h) => `#${h.toLowerCase()}`);

export const PALETTE_PRESETS: DrawPalette[] = [
  { id: 'mapforge', name: 'MapForge', colors: DRAW_SWATCHES, preset: true },
  { id: 'pico8', name: 'PICO-8', preset: true, colors: hexes('000000 1D2B53 7E2553 008751 AB5236 5F574F C2C3C7 FFF1E8 FF004D FFA300 FFEC27 00E436 29ADFF 83769C FF77A8 FFCCAA') },
  { id: 'sweetie16', name: 'Sweetie 16', preset: true, colors: hexes('1a1c2c 5d275d b13e53 ef7d57 ffcd75 a7f070 38b764 257179 29366f 3b5dc9 41a6f6 73eff7 f4f4f4 94b0c2 566c86 333c57') },
  { id: 'db16', name: 'DawnBringer 16', preset: true, colors: hexes('140c1c 442434 30346d 4e4a4e 854c30 346524 d04648 757161 597dce d27d2c 8595a1 6daa2c d2aa99 6dc2ca dad45e deeed6') },
  {
    id: 'endesga32',
    name: 'Endesga 32',
    preset: true,
    colors: hexes('be4a2f d77643 ead4aa e4a672 b86f50 733e39 3e2731 a22633 e43b44 f77622 feae34 fee761 63c74d 3e8948 265c42 193c3e 124e89 0099db 2ce8f5 ffffff c0cbdc 8b9bb4 5a6988 3a4466 262b44 181425 ff0044 68386c b55088 f6757a e8b796 c28569'),
  },
  { id: 'gameboy', name: 'Game Boy', preset: true, colors: hexes('0f380f 306230 8bac0f 9bbc0f') },
];

/** "#aabbcc" lines / .hex files (Lospec format) → colours */
export function parsePaletteText(text: string): string[] {
  const out: string[] = [];
  for (const m of text.matchAll(/#?\b([0-9a-fA-F]{6})\b/g)) out.push(`#${m[1].toLowerCase()}`);
  return [...new Set(out)];
}

/** lighten (+) or darken (−) a colour by amount 0..1 */
export function shiftColor(r: number, g: number, b: number, amount: number): [number, number, number] {
  if (amount > 0) return [r + (255 - r) * amount, g + (255 - g) * amount, b + (255 - b) * amount].map(Math.round) as [number, number, number];
  const f = 1 + amount;
  return [r * f, g * f * 0.97, b * f * 1.02].map((v) => Math.max(0, Math.min(255, Math.round(v)))) as [number, number, number];
}
