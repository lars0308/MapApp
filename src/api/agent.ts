...
async function step(messages: Message[]): Promise<{ content: Block[]; stop_reason: string }> {
  let r: Response;
  try {
    r = await fetch('/api/agent', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ messages }) });
  } catch {
    throw new Error('Keine Verbindung zum Server – bist du online?');
  }
  const out = (await r.json().catch(() => null)) as { ok: boolean; content?: Block[]; stop_reason?: string; error?: string } | null;
  ...
  return { content: out.content ?? [], stop_reason: out.stop_reason ?? 'end_turn' };
}
 ...
 */
export async function runAgent(task: string, images: { label: string; dataUrl: string }[] = []): Promise<string> {
  ...
  const first: Block[] = [];
  for (const im of images) first.push({ type: 'text', text: `${im.label}:` }, await imageBlock(im.dataUrl));
  ...
  try {
    for (let turn = 0; turn < MAX_TURNS; turn++) {
      ...
      const { content, stop_reason } = await step(messages);
      ...
      for (const call of calls) {
        ...
        if (!res.ok) parts.push({ type: 'text', text: res.error });
          ...
          if (res.binary?.kind === 'image') parts.push(await imageBlock(`data:${res.binary.mime};base64,${res.binary.base64}`, 1400));
