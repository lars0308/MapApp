...
export type Page = 'project' | 'map' | 'character' | 'creature' | 'object' | 'settings';
...
export type Chooser = 'map' | 'figures';
...
export const useApp = create<AppState>((set, get) => ({
  ...
  goToTab: (tab) => {
    ...
    const c: Chooser | null = tab === 'map' || tab === 'figures' ? tab : null;
    if (c && tabOf(get().page) === tab && get().chosen[c]) return set({ chosen: { ...get().chosen, [c]: false } });
    open(tab === 'figures' ? get().figure : tab);
  },
  goTo: (page) => {
    const c: Chooser | null = page === 'map' ? 'map' : (FIGURE_PAGES as string[]).includes(page) ? 'figures' : null;
