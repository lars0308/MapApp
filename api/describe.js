// POST /api/describe {text, image?, tileset?, context} → {ok, plan}
// Turns a description of the game / map (plus an optional reference picture and tileset) into a
// MapForge build plan via Claude on the Vercel AI Gateway. The app builds the map from the plan
// with its own generator (real tiles, collision, Godot export) – the model never draws the map.
...
export default async function handler(req, res) {
  ...
  if ((body.image?.length ?? 0) > MAX_IMAGE || (body.tileset?.length ?? 0) > MAX_IMAGE) return res.status(413).json({ ok: false, error: 'Bild zu groß' });
  if (!body.context?.generator) return res.status(400).json({ ok: false, error: 'context fehlt' });
  ...
  if (ref) content.push({ type: 'text', text: 'Referenzbild:' }, ref);
