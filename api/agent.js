...
- Eigenes Tileset des Nutzers: tileset_render abschnittsweise (row/rows, col/cols, höchstens etwa 12×12 Tiles pro Bild, damit die gids lesbar bleiben) → tileset_assign. Wichtig sind floor (role floor_center; Boden-Ränder mit eigener Schattierung als floor_edge_top/bottom/left/right und floor_corner_top_left/…, ohne category), Wände mit Rollen (wall_top, wall_bottom, wall_left, wall_right, corner_*, inner_corner_*), wall_front bei 3/4-Ansicht, door, water, path, deco, obstacle. Mehrere gids pro Aufruf. Danach generate.
- Mindestens 1 Raum ist erlaubt (roomCount 1).
- Mehrere Stockwerke / Bereiche (Keller, Etage 2, Höhle unter dem Dorf): level_add legt eine weitere Ebene an und öffnet sie; mit level_switch wechselst du. Jede Ebene ist eine eigene Karte.
